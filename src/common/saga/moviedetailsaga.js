...
export default function* CallApiMoviedetail({ payload }) {
  try {
    ...
    const datadetail = yield call(() =>
      Axios.get(`/movie/${payload}?api_key=${api_Key}&language=en-US`).then(
        response => response.data
      )
    );
    const videos = yield call(() =>
      Axios.get(`/movie/${payload}/videos?api_key=${api_Key}&language=en-US`).then(
        response => response.data
      )
    );

    const credits = yield call(() =>
      Axios.get(`/movie/${payload}/credits?api_key=${api_Key}&language=en-US`).then(
        response => response.data
      )
    );
    ...
    yield put(MoviedetailAction.requestMoviedetailError(error.response.data));
  }
}
